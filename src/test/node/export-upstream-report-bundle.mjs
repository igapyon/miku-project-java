...
if (!xmlPath || !outputZip) {
  console.error("usage: node src/test/node/export-upstream-report-bundle.mjs <input.xml> <output.zip>");
  process.exit(2);
}
...
try {
  ...
  fs.writeFileSync(outputZip, Buffer.from(reportBundle.zipBytes));
} finally {
  dispose();
}
