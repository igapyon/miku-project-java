...
- [ ] Java 版の first cut の目的を明文化する
- [ ] `MS Project XML` と `ProjectModel` の位置づけを Java 版でも定義する
- [ ] Java 版 `ProjectModel` の責務と範囲を決める
- [ ] first cut で扱う要素範囲を決める
- [ ] CLI を first cut に含めるか決める
- [ ] Node 版 upstream から継承する仕様と、Java 版で一旦保留する仕様を切り分ける

## Node -> Java 移植論点

- [ ] `ProjectModel` を `Map` ベースで持つか、POJO で持つか方針を決める
...
- [ ] `docs/` を作成する
- [ ] Java 版 STEP 1 仕様メモを書く
- [ ] `MS Project XML -> ProjectModel` の最小 import 設計を検討する
